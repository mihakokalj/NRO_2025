...
int main() {
  ...
  for (int ii = 0; ii < 2000; ii++) {
    ...
    /*
  Paralelizacija z OpenMP je bila izvedena z uporabo direktive #pragma omp parallel for.
  To omogoča, da se iteracija čez vse vrstice matrike izvede v več thredih.

  V tej implementaciji je paralelizacija možna, saj je vsak izračun elementa T[i] odvisen
  le od trenutnih vrednosti T in matrike A in to potem ne povzroča konflikotv med thredi

  Prednost paralelizacije je hitrejše izvajanje programov na večjedrnih procesorjih.
  Vendar paralelizacija morda ne bo znatno izboljšala hitrosti pri zelo majhnih matrikah
  ker gre potem veliko računske moči v sinhronizacijo teh treadov.
  */
